...
export type BaseBrushProps = {
  ...
  onVoronoiMouseMove?: (event: React.PointerEvent<SVGRectElement>) => void;
};
...
> {
  ...
  componentDidMount() {
    if (this.props.useWindowMoveEvents) {
      ...
      window.addEventListener(
        "mousemove",
        this.debouncedHandleWindowPointerMove
      );
    }
  }
            ...
            ...this.getExtent(
              {
                x:
                  brushingType === "left"
                    ? Math.min(
                        Math.max(x0 + offsetX, prevBrush.bounds.x0),
                        prevBrush.bounds.x1
                      )
                    : x0,
                y:
                  brushingType === "top"
                    ? Math.min(
                        Math.max(y0 + offsetY, prevBrush.bounds.y0),
                        prevBrush.bounds.y1
                      )
                    : y0,
              },
              {
                x:
                  brushingType === "right"
                    ? Math.min(
                        Math.max(x1 + offsetX, prevBrush.bounds.x0),
                        prevBrush.bounds.x1
                      )
                    : x1,
                y:
                  brushingType === "bottom"
                    ? Math.min(
                        Math.max(y1 + offsetY, prevBrush.bounds.y0),
                        prevBrush.bounds.y1
                      )
                    : y1,
              }
  ...
  handleDragStart = (draw: DragArgs) => {
    const { onBrushStart, left, top, inheritedMargin, useWindowMoveEvents } =
      this.props;
    ...
    this.updateBrush((prevBrush: BaseBrushState) => ({
      ...
      isBrushing: true,
      brushingType: "select",
      brushPageOffset: useWindowMoveEvents
        ? getPageCoordinates(draw.event)
        : undefined,
    }));
  };
      ...
      <Group className="visx-brush" top={top} left={left}>
        {disableDraggingOverlay ? (
          ...
          <Drag
            ...
            isDragging={
              useWindowMoveEvents ? brushingType === "select" : undefined
            }
          >
        ...
        {start && end && (
          <BrushSelection
            ...
            isDragInProgress={
              useWindowMoveEvents ? brushingType === "move" : undefined
            }
          />
        )}
        {/* handles */}
