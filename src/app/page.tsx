...
export default function Home() {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="p-4">
        <div className="flex justify-between items-center">
          ...
          {isDesktop && (
            <div className="header-state-county-selector">
              {/* State and county selector */}
            </div>
          )}
          {isDesktop && (
            <button className="switch-button">{/* Switch button */}</button>
          )}
        </div>
      </header>

      <main className="flex-grow flex flex-col desktop:flex-row border-4 m-4">
        <aside
          className={`  bg-gray-100 w-full  transition-all duration-300 ease-in-out ${
            isSideBarOpen ? "desktop:w-1/4" : "desktop:w-0 overflow-hidden"
          } space-y-4`}
          id="side-bar"
        >
          <Select>
          ...
          <h1 className="text-3xl font-bold mb-4">Reality- Perception Gap</h1>
            ...
            {categories.map((category, i) => (
              ...
              >
                <button
                  ...
                  } border-[lightgrey] w-full p-2 text-left font-semibold flex justify-between items-center`}
                >
                  <h3>{category.name}</h3>
                  <svg
        ...
        <section className="w-full desktop:w-1/2 p-4 flex flex-col">
          {isDesktop && (
  ...
  src="https://observablehq.com/embed/cadb3c4f5452d830?cells=viewof+selectedState%2Cmap"></iframe>
          </div>
